...
coefs = numpy.polyfit(Val3,Val2,2)
print("Coefficents for Datafit for Equation-Y=aX^2+bX+c are [a b c]=")
print("[a,b,c]=",coefs)
#coefs1=numpy.polyfit(math.log(Val1),math.log(Data[i,0]))
#ffit = numpy.poly1d(coefs)
print "\n"  
...
popt,pcov=curve_fit(func,dat,Val1)
print "Coefficents for exponent fit,Y=A*exp(B*X)",popt
print "\n"  
...
f=scipy.integrate.quad(fx,0,1,args=(coefs,))

print"fractional ability of bed [ability,error] is:", f
#f = scipy.integrate(Val2,Val3,0,1)
#print f
...
# From Eqn. 11.79:
Za = NtoG*HtoG;# [m]
print "adsorption zone height in meters is Za:",Za,"meters"
print "\n"
# From Eqn. 11.74:
Degree = (Z-(f*Za))/Z;
print "degree of saturation is:",Degree
print "\n"
...
print"Time to reach breakpoint is: ",round((Y1/(Y2*3600)),4)," hrs\n"
print ("\nTHANK YOU SIR")